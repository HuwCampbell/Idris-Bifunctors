...
(Bifunctor p, Functor f) => Bifunctor (Tanned f p) where
  bimap f g = Tannen . map (bimap f g) . runTannen

(Bifunctor p, Functor f) => Functor (Tanned f p a) where
  map f = Tannen . map (second f) . runTannen

(Biapplicative p, Applicative f) => Biapplicative (Tanned f p) where
  bipure a b = Tannen . pure $ bipure a b
  (Tannen fg) <<*>> (Tannen xy) = Tannen $ (map (<<*>>) fg) <*> xy

(Foldable f, Bifoldable p) => Bifoldable (Tanned f p) where
  bifoldMap f g = (foldr ((<+>) . (bifoldMap f g)) neutral) . runTannen

(Foldable f, Bifoldable p) => Foldable (Tanned f p a) where
  foldr f z t = applyEndo ((((concatMap . bifoldMap (const neutral)
                                                  $ Endo . f)) . runTannen) t) z

(Traversable f, Bitraversable p) => Bitraversable (Tanned f p) where
  bitraverse f g = map Tannen . traverse (bitraverse f g) . runTannen

(Traversable f, Bitraversable p) => Traversable (Tanned f p a) where
