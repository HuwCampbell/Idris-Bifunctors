...
(Bifunctor p, Functor f, Functor g) => Bifunctor (Biffed p f g) where
  bimap f g = Biff . bimap (map f) (map g) . runBiff

(Bifunctor p, Functor g) => Functor (Biffed p f g a) where
  map f = Biff . second (map f) . runBiff

(Biapplicative p, Applicative f, Applicative g) =>
         Biapplicative (Biffed p f g) where
  bipure a b                = Biff $ bipure (pure a) (pure b)
  (Biff fg) <<*>> (Biff xy) = Biff $ bimap (<*>) (<*>) fg <<*>> xy

(Bifoldable p, Foldable f, Foldable g) =>
