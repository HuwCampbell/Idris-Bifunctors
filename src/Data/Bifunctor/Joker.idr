...
Functor f => Bifunctor (Joked f) where
  bimap _ g = Joker . map g . runJoker

Functor f => Functor (Joked f b) where
  map g = Joker . map g . runJoker

Applicative f => Biapplicative (Joked f) where
  bipure                    = const $ Joker . pure
  (Joker a) <<*>> (Joker b) = Joker $ a <*> b

Foldable t => Bifoldable (Joked t) where
  bifoldMap _ g = concatMap g . runJoker

Foldable t => Foldable (Joked t a) where
  foldr f z = foldr f z . runJoker

Traversable t => Bitraversable (Joked t) where
  bitraverse _ g = map Joker . traverse g . runJoker

Traversable t => Traversable (Joked t a) where
